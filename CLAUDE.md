...
- Any edit to `.github/workflows/**` — especially `publish.yml`, the sole OIDC release path (`release.yml` was retired 2026-06-16).
...
- **Secrets**: never committed. `.gitignore` covers `.env*` patterns indirectly via `*.tsbuildinfo`/log entries; verify before `git add`. Publishing is tokenless via the OIDC trusted publisher (`publish.yml`) — no `NPM_TOKEN` secret exists or is needed.
- **License**: MIT. New deps must be MIT/ISC/BSD/Apache-2.0. Refuse copyleft.
- **Provenance**: `publish.yml` publishes with `--provenance --access public` (npm OIDC + Sigstore attestation). Do not remove the flag.
...
| Publish gate | GitHub Release event triggers `publish.yml` → tokenless OIDC npm publish (+ provenance) |
