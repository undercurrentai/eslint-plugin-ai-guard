...
- **Node.js** ≥ 20.12.0 (Node 18 is EOL as of 2025-04-30, and `@inquirer/prompts` — used by `ai-guard init-context` / `preset` — requires `node:util`'s `styleText`, introduced in Node 20.12.0; this is the `engines.node` floor).
