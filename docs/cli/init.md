...
Automatically configure `@undercurrentai/eslint-plugin-ai-guard` in your project. Detects your ESLint version, generates or patches your config file, and tells you exactly what to install if anything is missing.
...
```
  ...
  ✖  @undercurrentai/eslint-plugin-ai-guard not found
  ...
  Run this first:
    ...
    npm install --save-dev @undercurrentai/eslint-plugin-ai-guard
...
```javascript
import aiGuardPlugin from '@undercurrentai/eslint-plugin-ai-guard';
