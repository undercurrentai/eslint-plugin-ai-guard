...
- **New rule `require-server-action-auth`** — flags Next.js Server Actions (`'use server'`) that contain no visible authentication/authorization check. A Server Action is an exported `async` function in a file-level `'use server'` module, or an exported `async` function whose own body starts with `'use server'`; per the Next.js docs these are reachable via direct `POST`, so auth must be verified inside each one. Recognizes `auth()` / `currentUser()` / `getServerSession()` / `verifySession()` / `unauthorized()` / etc. (extend via `authCallers`). **`off` in `recommended`** (new + HIGH-false-positive-risk — Server Actions legitimately centralize auth in varied ways), `error` in `strict`, `warn` in `security`/`framework`. Conservative by construction: exported `async` functions only; an `assumeMiddlewareAuth` option (default `false`) opts out if you centralize auth in a gate the linter can't see — defaulting false because trusting an unseen middleware would convert a false positive into a missed-auth false negative. +15 RuleTester cases (10 valid incl. FP-guards for non-exported / sync / non-`'use server'` files, 5 false-negative guards).
- **`require-webhook-signature` recognizes more providers** (was Stripe / GitHub / Svix / Slack). Added on a **provenance-gated** basis — distinctive, webhook-specific method names accepted on sight: **Discord** (`verifyKey`), **Clerk** (`verifyWebhook`), **Square** (`isValidWebhookEventSignature`); and the generic verbs `verify` / `validate` / `verifySignature` accepted **only** when the receiver provably traces to a recognized webhook library (`svix`, `@octokit/webhooks`, `@sendgrid/eventwebhook`, `sns-validator`, `square`, `@linear/sdk`, `@clerk/backend`) via import binding, `new Ctor()`, or a one-hop `const x = new Ctor()` resolver — covering **SendGrid**, **AWS SNS**, **Square** (`verifySignature`), **Linear**, and Clerk's legacy svix path. This extends the existing anti-`jwt.verify` discipline to `.validate()` / `.verifySignature()` and **closes a latent false negative**: a bare `verify(...)` identifier call (e.g. `import { verify } from 'jsonwebtoken'`) was previously accepted unguarded and now correctly flags. Generic verbs are **never** matched by name alone (the name-based receiver fallback is restricted to `verify`), so a `webhooks`-named Joi schema calling `.validate()`, a forged local with a no-op `.validate()`/`verify()`, or a bare `validateRequest()` (express-validator) do **not** satisfy the rule. **Twilio (`validateRequest`) and Shopify (`shopify.webhooks.validate`) are intentionally NOT auto-recognized** — `validateRequest` collides with `express-validator` and Shopify needs member-chain-head provenance; add them via `verificationFunctions` until first-class support lands. +13 RuleTester cases (7 valid provider patterns + 6 false-negative guards). See the rule doc's *Known limitations*.

### Changed

- **Corrected `engines.node` floor `>=20.0.0` → `>=20.12.0`** to match what the CLI actually requires at runtime. The CLI's `@inquirer/prompts` → `@inquirer/core` chain does a top-level `import { styleText } from 'node:util'`, and `styleText` is only a named export of `node:util` from Node **20.12.0**. The prior `>=20.0.0` floor advertised a 20.0–20.11 support window in which `ai-guard init-context` / `preset` would crash at module load. (The CI Node-20 matrix leg resolves to the latest 20.x, so it never exercised that window — the gap was invisible to CI.) Tightening the declared floor turns a silent runtime crash into an honest `npm` engines warning.
- **Build target raised `node18` → `node20`** in `tsup.config.ts`, aligning the esbuild downlevel target with the Node floor (now `>=20.12.0`; `node20` is a conservative ≤-floor downlevel target). This is a strict relaxation of the downlevel floor, so it cannot break code that built under the stricter `node18` target; verified byte-identical bundle output (built both targets, diff clean) and the CJS interop footer, dual CJS/ESM output, and CLI shebang are unchanged.
- **Widen `peerDependencies.eslint` to `^9.0.0 || ^10.0.0`** to support ESLint 10 (released Feb 2026; flat-config-only, Node `^20.19.0 || ^22.13.0 || >=24`). Previously a consumer on `eslint@10` hit an `ERESOLVE` peer conflict on first install. The plugin loads and rules fire unchanged under ESLint 10 — `@typescript-eslint/parser`/`utils`/`rule-tester` at `^8` have permitted ESLint 10 since 8.56.0. Verified via an isolated `npm pack` + flat-config smoke (consumer `ERESOLVE` reproduced under the old `^9.0.0`, then confirmed resolved + plugin-loads + rule-fires + CLI-loads under an `eslint@10` runtime). A dedicated `eslint10` CI job installs `eslint@10` on Node 24 and runs typecheck + build + test; the existing Node 20/22/24 matrix stays on ESLint 9. (See the separate `engines.node` correction above: the floor is `>=20.12.0`; ESLint 10 itself requires Node `^20.19.0 || ^22.13.0 || >=24`, so consumers pairing this plugin with ESLint 10 are on Node ≥20.19 regardless.)
